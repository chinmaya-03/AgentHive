...
import { FolderGit2, AlertCircle, ArrowRight, Mail, Lock } from 'lucide-react';
...
const Login: React.FC = () => {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      const msg = extractErrorMessage(
        err,
        'Invalid email or password. Please check your credentials or verify backend server is running.'
      );
      setErrorMessage(msg);
  ...
  const handleDemoFill = () => {
    setValue('email', 'admin@agenthive.com');
        ...
        <div className="card-glass p-8 border border-slate-800 shadow-2xl">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="admin@agenthive.com"
          ...
          <div className="mt-4 flex items-center justify-between border-t border-slate-800/80 pt-4">
            <button
              ...
              className="text-xs text-indigo-400 hover:text-indigo-300 font-medium transition-all-custom"
            >
              Fill Demo Credentials
            </button>
            <span className="text-2xs text-slate-500">Fast testing mode</span>
          </div>

          {/* Redirect to register */}
          <div className="mt-4 pt-2 text-center text-sm">
            <span className="text-slate-400">Don't have an account? </span>
            <Link to="/register" className="font-semibold text-indigo-400 hover:text-indigo-300 transition-all-custom">
              Create an account
            </Link>
          </div>
        </div>
